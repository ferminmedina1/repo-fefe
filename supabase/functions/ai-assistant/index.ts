...
function buildBusinessContext(d: any, context?: string) {
  ...
  // Customer account movements — overdue
  ...
  // Quotation items — most quoted products
  ...
  // Return items — most returned products
...
- Últimas: ${d.quotations.slice(0, 5).map((q: any) => `#${q.id.slice(0, 8)} $${fmt(q.total || 0)} (${q.status})`).join(', ')}

── SERVICIOS TÉCNICOS ──
- Abiertos: ${openServices.length}
- Ingresos por servicios (completados): $${fmt(servicesRevenue)}
- Últimos: ${d.techServices.slice(0, 5).map((t: any) => `${t.device_type || 'Equipo'} - ${t.status} ($${fmt(t.total || 0)})`).join(', ')}

── DEVOLUCIONES (90d) ──
- Total devoluciones: ${d.returns.length} por $${fmt(returnsTotal)}
...
── PROVEEDORES ──
- Activos: ${d.suppliers.length}
- Compras (90d): ${d.purchases.length} órdenes
...
2. Sé conciso: Resumen Ejecutivo (2-3 líneas) → Insights (2-5 bullets con números) → Recomendación concreta → Próximo paso
3. Sustentá TODO con los datos reales provistos. Si no hay datos, decilo
4. Destacá riesgos urgentes al inicio (stock crítico, cashflow negativo, deuda alta)
5. Incluí siempre un KPI o métrica en cada insight
6. Cuando des recomendaciones, sé específico: producto, monto, plazo, acción concreta
7. No inventes datos — si necesitás más info, pedí 1 pregunta puntual
8. Si el usuario pregunta algo fuera de tu alcance de datos, sugerí dónde buscar

CAPACIDADES DE ANÁLISIS:
- Ventas: tendencias, proyección, ticket promedio, métodos de pago, mejor día de venta
- Inventario: stock crítico, rotación, dead stock, días de stock, reposición óptima
- Clientes: segmentación, VIP, riesgo abandono, deuda, frecuencia de compra
- Finanzas: P&L simplificado, cashflow, cuentas por cobrar/pagar, gastos por categoría
- Tesorería: saldos bancarios, cheques pendientes, tarjetas por acreditar
- RRHH: dotación, comisiones, horas trabajadas, costo laboral, última liquidación
- Cotizaciones: pipeline comercial, cotizaciones vencidas, tasa de conversión
- Servicios técnicos: órdenes abiertas, ingresos por servicios, prioridades
...
ACCIONES DISPONIBLES (si el usuario pide ejecutar algo, indicá el JSON de acción):
Podés sugerir acciones que el usuario puede ejecutar desde la plataforma:
- Crear alerta de stock: {"action": "navigate", "path": "/inventory-alerts"}
- Ver reportes: {"action": "navigate", "path": "/reports"}
- Ver clientes con deuda: {"action": "navigate", "path": "/accounts-receivable"}
- Gestionar cotizaciones: {"action": "navigate", "path": "/quotations"}
- Ver servicios técnicos: {"action": "navigate", "path": "/technical-services"}
- Ver caja: {"action": "navigate", "path": "/cash-register"}
- Ver empleados: {"action": "navigate", "path": "/employees"}
- Ver comisiones: {"action": "navigate", "path": "/commissions"}
- Ver nómina: {"action": "navigate", "path": "/payroll"}
- Ver gastos: {"action": "navigate", "path": "/expenses"}
- Ver promociones: {"action": "navigate", "path": "/promotions"}
- Ver devoluciones: {"action": "navigate", "path": "/returns"}
- Ver depósitos: {"action": "navigate", "path": "/warehouses"}
- Ver bancos: {"action": "navigate", "path": "/bank-accounts"}
...
Cuando sugieras una acción, insertá al final de tu respuesta una línea con formato:
[ACTION:{"action":"navigate","path":"/ruta"}]
Solo incluí la acción si es directamente relevante a lo que el usuario pidió.`;

  switch (type) {
    case 'stock-analysis':
      prompt += `\n\nFOCO: Inventario. Priorizá productos críticos, sugerí cantidades de reposición con base en velocidad de venta, detectá dead stock y capital inmovilizado.`;
      break;
    case 'sales-prediction':
      prompt += `\n\nFOCO: Predicción de ventas. Usá patrones de días de semana, tendencia 7d vs 30d, y proyectá la próxima semana. Indicá supuestos.`;
      break;
    case 'customer-insights':
      prompt += `\n\nFOCO: Clientes. Segmentá en VIP/regulares/inactivos, detectá riesgo de abandono, calculá valor de vida, sugerí tácticas de retención.`;
      break;
    case 'financial-summary':
      prompt += `\n\nFOCO: Finanzas. P&L ejecutivo, cashflow, cuentas por cobrar/pagar, cheques/tarjetas pendientes, gastos por categoría, recomendación.`;
      break;
    case 'hr-analysis':
      prompt += `\n\nFOCO: RRHH. Dotación, costo laboral, comisiones, productividad (horas/ventas), ausentismo, última liquidación.`;
      break;
    case 'treasury':
      prompt += `\n\nFOCO: Tesorería. Saldos bancarios, cheques pendientes, tarjetas por acreditar, flujo de fondos proyectado.`;
      break;
    case 'crm-pipeline':
    ...
    case 'support-analysis':
      ...
      break;
    default:
      prompt += `\n\nRespondé la consulta usando todos los datos disponibles. Priorizá insights accionables.`;
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const { query, type, companyId, context, conversationHistory, conversationId } = await req.json();
    ...
    console.log('AI Assistant request:', { type, queryLen: query?.length, tables: Object.keys(bizData).length });

    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${LOVABLE_API_KEY}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: 'google/gemini-2.5-flash', messages, max_tokens: 3000 }),
    });

    if (!response.ok) {
      if (response.status === 429) return new Response(JSON.stringify({ error: 'Límite de solicitudes alcanzado. Intenta en unos minutos.' }), { status: 429, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    ...
    // Save conversation to history if conversationId provided
    if (conversationId) {
      try {
        await supabaseClient.from('ai_conversations' as any).upsert({
          id: conversationId,
        ...
        // Save messages with rich metadata
        await supabaseClient.from('ai_conversation_messages' as any).insert([
          {
          ...
          },
          {
            ...
            metadata: {
              ...
              char_count: cleanResponse.length,
              has_actions: !!action,
              action_suggested: action?.path || null,
              prompt_tokens: usage.prompt_tokens || null,
              completion_tokens: usage.completion_tokens || null,
              total_tokens: usage.total_tokens || null,
              company_id: companyId,
              response_timestamp: new Date().toISOString(),
              tables_queried: Object.keys(bizData).length,
            },
          },
        ] as any);
      } catch (e) {
